package ac.cn.origind.lrcv2toass
...
fun main(args: Array<String>) {
    if (args.isEmpty()) {
        println("Usage: java -jar <this_program> <LRCv2_File> [encoding] [k-time-cmd]")
        return
    }
    var encoding = "UTF-8"
    var kCmd = "k"
    if (args.size > 1) encoding = args[1]
    if (args.size > 2) kCmd = args[2].removePrefix("\\")

    val fileText = File(args[0].removeSurrounding("\"")).readText(Charset.forName(encoding)).replace("\r\n", "\n")
    val tokens = ArrayList<RawToken>()

    fileText.lines().forEach { line ->
...
""")
    ...
    println(res)
    File("E:\\tttttt.ass").writeText(res)
}
